...
const BENCH=[
 {name:'GPQA Diamond',capability:'Reasoning',values:[94.1,92.9,88.1,92.4,89.9]},
 {name:'Terminal Bench 2.1',capability:'Agentic terminal coding',values:[73.0,63.4,64.0,51.7,78.2]},
 {name:'SWE-bench Pro',capability:'Agentic coding',values:[61.7,53.5,57.6,51.2,53.4]},
 {name:'NL2Repo-Bench',capability:'Repo-level generation',values:[42.3,36.2,41.1,null,47.6]},
 {name:'DeepSWE 1.1',capability:'Agentic coding',values:[42.2,13.3,14.2,null,null]},
 {name:'QwenSWEBench',capability:'Software engineering',values:[79.0,49.3,59.2,null,63.8]},
 {name:'CoWorkBench',capability:'Long-horizon office work',values:[70.7,61.0,65.1,null,null]}
];
const BENCH_MODELS=['Qwen3.8-27B','Qwen3.6-27B','Qwen3.7-Plus','Muse Glimmer-30B','Opus4.6 Max'];
...
LINE_FIXES: list[tuple[int, str, str]] = [
    ...
    (806, """ renderBench(){const s=this.state,b=BENCH[s.bench];return html`<div><div class="inspect-path">REPORTED EVALUATIONS / QWEN3.8-27B</div><h2 class="editorial">Capability,<br/><em>as reported.</em></h2><p class="lede">Every number below is copied from the Qwen3.8-27B model card that ships with this checkpoint. Cells the card leaves blank stay blank here — nothing is inferred and nothing is an estimate. """, "bench head"),
    (807, """ <${Section} title=${b.name} tag=${b.capability.toUpperCase()}><div class="benchmark-chart">${BENCH_MODELS.map((name,i)=>html`<div class="benchmark-row${i===0?' featured':''}"><div><span>${name}</span><b>${b.values[i]==null?'—':b.values[i]}</b></div><div class="bench-track"><i style=${{width:(b.values[i]||0)+'%',background:i===0?COL.dec:COL.muted}}/></div></div>`)}</div><${Fact} label="Reasoning mode" value="thinking on by default · reasoning_effort tunable"/><${Fact} label="Historical reasoning" value="kept with preserve_thinking"/><p class="fine">Reported with thinking enabled at high effort. Turning thinking off, or lowering the effort, moves these numbers — treat the table as one operating point, not a ceiling.</p><//>""", "bench rows"),
    (808, """ <${Section} title="The other columns"><p class="fine">Qwen3.6-27B is the direct predecessor at the same size; Qwen3.7-Plus and Muse Glimmer-30B are the adjacent open models the card compares against, and Opus4.6 Max is the closed reference. This page does not score the checkpoint itself — it only shows the checkpoint's own bytes.</p><${Fact} label="Context" value="262,144 native · 1,000,000 extended"/><${Fact} label="Modalities" value="text · image · video"/><${Fact} label="Licence" value="Apache-2.0"/><//></div>`}""", "bench footer"),
